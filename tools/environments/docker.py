...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        logger.info("ockerEnvironment volumes: %s", volumes)
        # Ensure volumes is a list (config.yaml could be malformed)
        if volumes is not None and not isinstance(volumes, list):
            logger.warning("ocker_volumes config is not a list: %r", volumes)
            volumes = []
        ...
        for vol in (volumes or []):
            if not isinstance(vol, str):
                logger.warning("ocker volume entry is not a string: %r", vol)
                continue
            vol = vol.strip()
            if not vol:
                continue
            if ":" in vol:
                ...
                logger.warning("ocker volume '%s' missing colon, skipping", vol)
        ...
        if auto_mount_cwd and host_cwd and not os.path.isdir(host_cwd_abs):
            logger.debug("kipping docker cwd mount: host_cwd is not a valid directory: %s", host_cwd)
        ...
        if bind_host_cwd:
            logger.info("ounting configured host cwd to /workspace: %s", host_cwd_abs)
        ...
        logger.info("ocker volume_args: %s", volume_args)
        ...
        logger.info("ocker run_args: %s", all_run_args)
        ...
        if not reused:
            ...
            logger.debug("tarting container: %s", ' '.join(run_cmd))
            try:
                result = subprocess.run(
                    run_cmd,
                    capture_output=True,
