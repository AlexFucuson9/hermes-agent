            ...
            try:
                ...
                if response_invalid:
                    ...
                    if retry_count >= max_retries:
                        ...
                        logger.error("agent.log_prefix}Invalid API response after %s retries.", max_retries)
                        agent._persist_session(messages, conversation_history)
                        return {
                            "messages": messages,
                            "completed": False,
                    ...
                    logger.warning("nvalid API response (retry %s/%s): %s | Provider: %s", retry_count, max_retries, ', '.join(error_details), provider_name)
                ...
                if is_payload_too_large:
                    ...
                    if compression_attempts > max_compression_attempts:
                        ...
                        logger.error("agent.log_prefix}413 compression failed after %s attempts.", max_compression_attempts)
                        agent._persist_session(messages, conversation_history)
                        return {
                            "messages": messages,
                            "completed": False,
                    ...
                    if len(messages) < original_len or (new_tokens > 0 and new_tokens < original_tokens * 0.95):
                        ...
                        agent._vprint(f"{agent.log_prefix}   💡 Try /new to start a fresh conversation, or /compress to retry compression.", force=True)
                        logger.error("agent.log_prefix}413 payload too large. Cannot compress further.")
                        agent._persist_session(messages, conversation_history)
                        return {
                            "messages": messages,
                            "completed": False,
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        if compression_attempts > max_compression_attempts:
                            ...
                            agent._vprint(f"{agent.log_prefix}   💡 Try /new to start a fresh conversation, or /compress to retry compression.", force=True)
                            logger.error("agent.log_prefix}Context compression failed after %s attempts.", max_compression_attempts)
                            agent._persist_session(messages, conversation_history)
                            return {
                                "messages": messages,
                                "completed": False,
                ...
                if is_client_error:
                    ...
                    logger.error("agent.log_prefix}Non-retryable client error: %s", api_error)
